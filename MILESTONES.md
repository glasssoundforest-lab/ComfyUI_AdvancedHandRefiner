...
**残る限界**: `sam2_hiera_tiny`自体が256×256という比較的低い解像度でしか
予測しないため、元画像の解像度が高いほど「拡大による粗さ」は完全には
解消しない（モデル自体の仕様上の限界）。より高精度な仕上がりが必要な
場合は、SAM2は輪郭の大まかな精密化の補助として使い、`finger_sharpness`
（骨格線ベースの補正）や通常のマスク編集と組み合わせることを推奨する。

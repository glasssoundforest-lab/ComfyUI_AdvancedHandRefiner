...
def estimate_finger_count_skeleton(
    ...
    # 近接する端点同士をまとめて1本として数える。
    #
    # ★改善（2026-07-07）: 以前は各端点を既存クラスタの中心とだけ
    # 比較する単純な貪欲法だったが、これは骨格上でジグザグに連なった
    # 一連の端点（1本の指の同じ根元付近から生じた、骨格化のブレによる
    # 複数の端点）を、点の走査順序次第では正しく1つにまとめきれない
    # ことが実データで確認された（本来1本の指が2本と数えられてしまう）。
    # 代わりに、各端点をmerge_dist分だけ膨張させた上で連結成分を数える
    # 方式に変更した。これにより、直接は離れていても膨張後に繋がる
    # （＝連鎖的に近接している）点群は走査順序によらず確実に1つの
    # クラスタとしてまとめられる。
    # 近接する端点同士をまとめて1本として数える(単純な貪欲クラスタリング)。
    #
    # ★試行錯誤の記録（2026-07-07）: 一時的に、各端点をmerge_dist分だけ
    # 膨張させて連結成分を数える方式（走査順序に依存しない、より
    # 「正確」に見えるクラスタリング）を試したが、これは「際どい間隔で
    # 挿入された余分な指」の検出（このモジュールの主要な存在意義であり、
    # 既に合成データで厳密に検証済みの能力）を退行させることが判明した
    # ため、元の貪欲法に戻した。理由: 膨張ベースの連結成分方式は、
    # 本来は別々の指である2本の指先の端点同士も、その間の骨格経路上に
    # 中間的な端点が連鎖していると、意図せず1つに統合してしまう
    # （「同じ指のジグザグ状の重複端点」と「隣接する別々の指の端点」を、
    # 単純な距離ベースの連結性だけでは区別できないため）。既に実データ・
    # 合成データの両方で検証済みの挙動を壊してまで採用する価値はないと
    # 判断し、単純だが実績のある貪欲法を維持することにした。
